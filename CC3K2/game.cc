...
std::string Controller::runGame(char type) {
	...
	while (true) {
		...
		if (validDirection(str)) {
			...
			std::cin >> str;
			if (validDirection(str)){
				log += floor->talkMerchant(str);
				if (log == "Invalid input." || log == "You are my enemy!"){
					display->printFloor(floor, log);
					continue;
				} else if (log == "(B)uckler Shield: 3g (T)arge Shield: 6g (P)avise Shield: 12g (N)odachi: 3g (K)atana: 7g (U)chigatana: 15g"){
					display->printFloor(floor, log);
					std::cin >> str;
					log = floor->buyMerchant(str);
					if (log == "Sorry I don't have that item. " || log == "You are too poor. "){
						display->printFloor(floor, log);
						continue;
					}
				} else {
